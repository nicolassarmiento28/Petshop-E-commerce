...
> Actualizado y verificado línea por línea contra el código (2026-07): la paleta cambió de un tinte **azulado** a un tinte **verde oliva/musgo oscuro**. Los 5 tokens se migraron en TODO el sitio (no solo en los componentes tocados originalmente) — ver la nota de migración completa más abajo.
...
|---|---|---|
| `dark-bg` (`bg-dark-bg`) | `#0e100c` | Fondo de página — `Layout.tsx` (raíz de toda la app), `Home.tsx` (root + Hero), `AdminLogin.tsx` (panel del formulario), `AdminLayout.tsx` (main), `CartDrawer.tsx`, `CheckoutPage.tsx`, `PaymentSuccess.tsx`, `PaymentFailed.tsx`, fallback `.dark body` en `index.css` |
| `dark-surface` (`bg-dark-surface`) | `#181a13` | Nivel 1 — `ProductCard.tsx`, `Home.tsx` (carrusel de marcas, cards de categoría, botón "Ver ofertas"), `AdminLogin.tsx` (tarjeta del formulario), `Footer.tsx`, `AdminDashboard.tsx` y demás vistas de admin (tarjetas/paneles), `PaymentSuccess.tsx`/`PaymentFailed.tsx` (tarjeta) |
| `dark-surface-elevated` (`bg-dark-surface-elevated`) | `#20231a` | Nivel 2 — `Navbar.tsx` (header sticky, menú mobile, inputs de búsqueda, hovers de íconos), `AdminLogin.tsx` (inputs y bloque de credenciales), `Home.tsx` (hover del botón "Ver ofertas"), inputs/hovers en todas las páginas de admin migradas |
| `dark-border` (`border-dark-border`) | `#282b1e` | Borde por defecto — en todos los componentes migrados (`Navbar`, `Home`, `ProductCard`, `AdminLogin`, `Footer`, `AdminLayout`, páginas de admin, `CartDrawer`/`CartItem`/`CartSummary`, `CheckoutPage`/`CheckoutForm`) |
| `dark-border-hover` (`border-dark-border-hover`) | `#3a3f2b` | Borde en hover/focus — `ProductCard.tsx` (hover de tarjeta completa), `AdminLogin.tsx` (hover de inputs) |

`dark.text` (`#e8eaf0`) y `dark.muted` (`#8892a4`) se mantienen sin cambio de valor — no son parte de este set de 5 tokens y no cambiaron de tinte.

**Migración completa a nivel de sitio (2026-07):** una revisión posterior encontró que 19 archivos aún usaban el set de hex neutro **anterior a los tokens** (`#111111` / `#1a1a1a` / `#222222` / `#2a2a2a`) — `Footer.tsx`, `AdminLayout.tsx`, `CategoryPage.tsx`, `ProductPage.tsx`, `AllProductsPage.tsx`, `CartPage.tsx`, `PaymentFailed.tsx`, `BrandsCarousel.tsx`, `SizeSelector.tsx`, y todas las páginas de admin (`AdminProducts`, `AdminOrders`, `AdminBrands`, `AdminCustomers`, `AdminCoupons`, `AdminQr`, `AdminDashboard`). Se migraron todos a los tokens `dark-bg`/`dark-surface`/`dark-surface-elevated`/`dark-border`, incluyendo un `contentStyle` inline de Recharts en `AdminDashboard.tsx` (tooltip del gráfico de ingresos, no es una clase Tailwind así que se actualizó el hex directamente a `#181a13`/`#282b1e`). **Ya no hay páginas con un tono de fondo distinto al resto en dark mode** — verificado visualmente en Home, todas las categorías, catálogo, ProductPage, carrito, checkout, login admin, dashboard admin y páginas de pago éxito/fallo.

> ⚠️ Pendiente menor sin resolver (no es de esta migración): `AdminLogin.tsx` sigue usando `dark:text-zinc-100`/`dark:text-zinc-400` (paleta `zinc` de Tailwind) en vez de `dark.text`/`dark.muted` para el texto — es una inconsistencia de nomenclatura, no de tono visual (los valores de zinc-100/400 se ven prácticamente iguales a `#e8eaf0`/`#8892a4`), así que no se corrigió por no ser parte de ningún pedido explícito todavía.

**Principio de diseño (aplicado):** nunca un solo negro plano para todo el dark mode — página (`dark-bg`), superficie/tarjeta (`dark-surface` o `dark-surface-elevated`) e inputs deben diferenciarse en tonos progresivamente más claros para dar sensación de profundidad. Azul (`blue-600`) y naranjo (`orange-500`) no cambian de familia en dark mode salvo los casos puntuales ya documentados: el gradiente `blue-900 → blue-700` y el glow naranjo (`bg-orange-500/30 blur-2xl`) del panel de marca en `AdminLogin.tsx` y `AdminLayout.tsx` (sidebar), y el degradado radial azul del Hero en `Home.tsx`. Los colores semánticos de estado (verde=éxito, ámbar=pendiente, rojo=cancelado) tampoco cambian con este tinte.
